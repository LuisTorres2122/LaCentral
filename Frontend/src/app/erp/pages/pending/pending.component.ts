...
export class PendingComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.chargeData();
    this.titleService.setTitle('Ordenes Pendientes');
  }
  ...
  updateOrder(): void {
    ...
    this.orderService
      .updatePendingorder(updatedData.idPedido, updatedData)
      .subscribe({
        next: (res) => {
  ...
  findIndexList(element: any): number {
    return this.elements.findIndex((el) => el.idPedido == element.idPedido);
  }
